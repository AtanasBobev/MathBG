...
<body style='display:none;' onunload='unload();SpecialServer()' onload="load()">
        ...
        <div id='advancedPrint'>
            <center>
                <fieldset>
                    ...
                    <input placeholder="My problems" value='Test 1' class="settings et">
                </fieldset>
                <fieldset>
                    <legend>Custom footer</legend>
                    <input placeholder="That's all folks!" value='Thank you for your time' class="settings et">
                </fieldset>
                <fieldset>
    ...
    const changeLanguage = (to) => {
        if (to == 'bg') {
            try {
                ...
                document.querySelector("#TextDiv > center > center").innerText = 'Натисно приложи, за да запазиш сегашните сетове';
    ...
    function SpecialServer() {
        if (navigator.onLine) {
            ...
            $('#SettingsPanel').show();
            if (currentLang == 'en') {
                alertify.alert('Error', 'No interent connectivity, we cannot save your data.');
            } else {
                alertify.alert('Грешка', 'Няма интернет връзка, не можем да запазим данните ви.');
            }
        }
    }
    ...
    window.onbeforeunload = function() {
        localStorage.setItem("temporaryData", ("AllProblemsCount=" + AllProblemsCount + ";ProblemsLeft=" + ProblemsLeft + ";timeleft=" + timeleft + ";game=" + game + ";MaximumGeneratedNumber=" + MaximumGeneratedNumber + ";MinimumGeneratedNumber=" + MinimumGeneratedNumber + ";num1=" + num1 + ";num2=" + num2 + ";sign='" + sign + "';result=" + result + ";datar='" + document.querySelector('#expression').innerText + "';document.querySelector('#expression').innerText='" + document.querySelector("#expression").innerText + "';startCountdownAll(" + pointer + ")"));
    ...
    window.onload = function() {
        ...
        if (localStorage.getItem('temporaryData') != null) {
            ...
            if (game == 'true' || game) {
                ...
                document.querySelector("#expression").innerText = datar;
                if (exitsAction == 1) {
                    // ("this line");
                    PrepareGame(false);
                    BeforeEval(localStorage.getItem("temporaryData"));
    ...
    setInterval(checkSecurity(), 3000);
    ...
    function checkSecurity() {
        if (!inIframe()) {
            ...
            $('html').show()
            if (currentLang == 'en') {
                alert("Running the application without a secured frame is not allowed! Go to https://sfconsult.net/MathBG/");
            } else {
                alert("Използването на приложението без защитен прозорец не е позволено! Отиди на https://sfconsult.net/MathBG/");
            }
            close();
            window.close();
            window.top.close();
            ...
            window.location.replace = 'https://www.sfconsult.net/MathBG/index.html';
            document.write("Running the application without a secured frame is not allowed! Go to https://sfconsult.net/MathBG/")
        }
    }
    </script>
